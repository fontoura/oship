...
__revision__ = "$Id: _clips_wrap.py 335 2008-01-13 01:50:50Z Franz $"
...
# check Python version, and issue an exception if below 2.3
if _sys.version[:3] < "2.3":
    raise _c.ClipsError("M99: Python 2.3 or higher required")
...
class Module(object):
    ...
    def __init__(self, o):
        ...
        if _c.isDefmodule(o): self.__defmodule = o
    ...
    def BuildInstance(self, name, defclass, overrides=""):
        ...
        mname = self.Name
        cmdstr = "(%s::%s of %s %s)" % (mname, name, str(defclass), overrides)
        return Instance(_c.makeInstance(cmdstr))
    def PrintInstances(self, classname=None):
        """print Instances to standard output"""
        _c.routerClear("temporary")
        if classname:
            _c.instances("temporary", self.__defmodule, str(classname), False)
        else: _c.instances("temporary", self.__defmodule)
        s = _c.routerRead("temporary")
        if s:
            _sys.stdout.write(s)
    def PrintSubclassInstances(self, classname):
        """print Instances to standard output"""
        _c.routerClear("temporary")
        _c.instances("temporary", self.__defmodule, str(classname), True)
        s = _c.routerRead("temporary")
        if s:
            _sys.stdout.write(s)
...
class Generic(object):
    ...
    def __init__(self, o):
        ...
        if _c.isDefgeneric(o): self.__defgeneric = o
        else: raise _c.ClipsError("M01: cannot directly create Generic")
    ...
    def AddMethod(self, restrictions, actions, midx=None, comment=None):
        ...
        if midx:
            ...
            indstr = ""
        if type(restrictions) in [list, tuple]:
            rstr = ""
...
class Class(object):
    ...
    def __init__(self, o):
        ...
        class __class_Slots:
            """define a structure for Class Slots"""
            def __init__(self, o): self.__defclass = o
            def __getstate__(self):
...
def PrintInstances(classname=None):
    ...
    _c.routerClear("temporary")
    if classname:
        _c.instances("temporary", str(classname), False)
    else:
...
def PrintSubclassInstances(classname):
    ...
    _c.routerClear("temporary")
    if classname:
        _c.instances("temporary", str(classname), True)
    s = _c.routerRead("temporary")
    if s:
        _sys.stdout.write(s)
#}}
