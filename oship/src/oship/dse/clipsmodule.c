 ...
 * $Id: clipsmodule.c 335 2008-01-13 01:50:50Z Franz $
...
#define clips_environment_New(p) \
    do { \
        ...
        if(p) { \
            ...
            memset(p->clips_StrayFacts, 0, sizeof(p->clips_StrayFacts)); \
        } \
    } while(0)
...
#define clips_environment_New(p) \
    do { \
        p = PyObject_New(clips_EnvObject, &clips_EnvType); \
        if(p) clips_environment_valid(p) = TRUE; \
    } while(0)
...
#define clips_fact_lock(v) ((clips_FactObject *)(v))->locked = TRUE
#define clips_fact_unlock(v) ((clips_FactObject *)(v))->locked = FALSE
...
#define clips_fact_New(e, p) do { \
        ...
        ((clips_FactObject *)(p))->locked = FALSE; \
    } while(0)
...
#define COPY_ADDITIONAL_ENVIRONMENT_DATA(_pe) do \
    { \
        ...
        memcpy(clips_StrayFacts, (_pe)->clips_StrayFacts, \
            sizeof(clips_StrayFacts)); \
    } while(0)
#define INJECT_ADDITIONAL_ENVIRONMENT_DATA(_pe) do \
    { \
        ...
        memcpy((_pe)->clips_StrayFacts, clips_StrayFacts, \
            sizeof(clips_StrayFacts)); \
    } while(0)
...
#define clips_instance_lock(v) ((clips_InstanceObject *)(v))->locked = TRUE
#define clips_instance_unlock(v) ((clips_InstanceObject *)(v))->locked = FALSE
